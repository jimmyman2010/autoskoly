...
{loop as $post}
	...
	{* DATE ICON ****** *} {if $wp->isSingular(post)} 			{var $dateIcon = $post->rawDate}  		{var $dateLinks = 'no'}		{var $dateShort = 'yes'} {/if}
	...
	{* EVENT PRO ICON ****** *} {if $wp->isSingular(event-pro)}
									...
									{capture $eventDateIcon}
										{includePart portal/parts/single-event-date, meta => $eventProMeta, place => 'header', eventId => $post->id}
									{/capture}
								{/if}
