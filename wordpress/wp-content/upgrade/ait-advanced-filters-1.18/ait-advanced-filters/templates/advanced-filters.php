...
if($advancedFiltersOptions->enabled){
	...
	?>
		<div class="advanced-filters-wrap">		
			<h2><?php _e('Advanced Filters', 'ait-advanced-filters') ?></h2>
							...
							<ul class="advanced-filters columns-<?php echo $advancedFiltersOptions->filterColumns ?>">
								<?php foreach($filters_avalaible as $filter){
									if(!empty($filter)){
										...
										switch($filter_type){
											case 'checkbox':
												echo '<li class="filter-container filter-checkbox '.$li_class.' "><input type="checkbox" name="'.$filter->slug.'" value="'.$filter->term_id.'" '.$in_checked.'><label><i class="fa fa-times"></i></label><span class="filter-name">'.$filter->name.'</span></li>';
											break;
											default:
