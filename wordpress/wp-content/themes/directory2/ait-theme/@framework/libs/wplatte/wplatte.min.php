...
WpLatte {
    ...
    static $params = array();
    protected
    static $translationsForDomain;
    protected
    static $templatesStorage = array();
    protected
    static $entitiesClasses = array(
    ...
    static
    function
    templateIncludeCallback($template) {
        self::render($template);
    }

    static
    function
    woocommerceTemplateIncludeCallback($templateName, $templatePath, $located, $args) {
    ...
    commentsTemplateCallback($commentsTemplate) {
        ...
        self::createTemplate($template, array('post' => self::createEntity('Post', $post)) + $layoutParams)
            ->render();
        return $blank;
    }
    ...
    static
    function
    searchFormTemplateCallback($form) {
        $layoutParams = apply_filters('wplatte-layout-params', self::$params);
        $templates = array(
        ...
        $template = locate_template($templates);
        $return = (string) self::createTemplate($template, $layoutParams)
            ->render();
        return $return;
    }
    ...
    static
    function
    renderHeaderFooterSidebarTemplate($template, $name) {
        $layoutParams = apply_filters('wplatte-layout-params', self::$params);
        $templates = array();
        if (!empty($name)) {
        ...
        self::createTemplate(self::$templatesStorage[$template][$name], $layoutParams)
            ->render();
    }

    static
    function
    removeRawIncludedContent($condition, $content, $template) {
        if ($condition) {
    ...
    createTemplateEntities() {
        ...
        elseif (is_home()) {
            ...
            if (isset($obj->post_status) or $obj
                instanceof
                WP_Post
            ) {
    ...
    static
    function
    createTemplate($template, $params = array()) {
    ...
    setupTemplating() {
        ...
        $template->setCacheStorage(new
        NPhpFileStorage(realpath(self::$config['cacheDir'])));
    ...
    static
    function
    createLatteFilterCallback($template) {
        $latte = new
        NLatteFilter;
    ...
    static
    function
    createEntity($entity, $params = NULL) {
        if (isset(self::$entitiesClasses[$entity])) {
            $class = self::$entitiesClasses[$entity];
        }
        else {
            trigger_error("There isn't specified class for entity '{$entity}'", E_USER_ERROR);
        }
        ...
        switch ($entity) {
            ...
            case'CommentAuthor':
                return
                    new$class($params[0], $params[1]);
            case'Attachment':
                return
                    new$class($params[0], $params[1], $params[2]);
            case'Wp':
                return
    ...
    static
    function
    gettextCallback($translated, $text, $domain) {
        if ($domain == 'wplatte') {
    ...
    static
    function
    gettextWithContextCallback($translated, $text, $context, $domain) {
        if ($domain == 'wplatte') {
    ...
    static
    function
    ngettextCallback($translated, $single, $plural, $number, $domain) {
        if ($domain == 'wplatte') {
    ...
    static
    function
    ngettextWithContextCallback($translated, $single, $plural, $number, $context, $domain) {
        if ($domain == 'wplatte') {
    ...
    function
    fixSignupPage() {
        $obStartCb = create_function('', 'ob_start();');
        add_action('wp', $obStartCb, 2000);
        add_action('before_signup_form', array(
            __CLASS__,
            'fixHeaderOnSignupPage'
        ), 0);
        add_action('after_signup_form', $obStartCb, 0);
    ...
    fixHeaderOnSignupPage() {
        ...
        echo
        self::removeRawIncludedContent(did_action('get_header'), $headerHtml, 'header');
        do_action('wplatte-after-signup-header');
    }

    static
    function
    fixFooterOnSignupPage() {
        $footerHtml = ob_get_clean();
        do_action('wplatte-before-signup-footer');
        echo
        self::removeRawIncludedContent(did_action('get_footer'), $footerHtml, 'footer');
    }

    static
    function
    homeUrl($isSearch = FALSE) {
        global $polylang;
        if (function_exists('PLL')) {
            return !empty(PLL()->links) ? PLL()->links->get_home_url(pll_current_language(), $isSearch) : home_url('/');
        }
        if (isset($polylang)) {
            return !empty($polylang->links) ? $polylang->links->get_home_url(pll_current_language(), $isSearch) : home_url('/');
        }
        return
            home_url('/');
    }
...
WpLatteBaseEntity
    ...
    function&__get($name) {
        ...
        if (method_exists($this, $name) and
...
WpLatteAttachmentEntity
    ...
    function
    __construct($postId, $parentId, $mimeType) {
        $this->id = $postId;
    ...
    isImage() {
        return
            preg_match('!^image/!', $this->mimeType);
    }

    function
    isVideo() {
        return
            preg_match('#^video/#', $this->mimeType);
    }

    function
    isAudio() {
        return
            preg_match('#^audio/#', $this->mimeType);
    }
    ...
    image($size = 'thumbnail') {
        return
            wp_get_attachment_image($this->id, $size);
    }
    ...
    video() {
        ...
        $shortcode = sprintf('[video src="%s"%s%s]', $this->url(), empty($meta['width']) ? '' : sprintf(' width="%d"', $w), empty($meta['height']) ? '' : sprintf(' height="%d"', $h));
        return
    ...
    audio() {
        return
            do_shortcode('[audio src="' . $this->url() . '"]');
    }
}
...
WpLatteCategoryEntity
    ...
    title() {
        return
            apply_filters('single_cat_title', $this->rawTitle);
    }

    function
    description() {
        return
            category_description($this->id);
    }
...
WpLatteCommentEntity
    ...
    function
    __construct($comment, $postAuthorId) {
    ...
    text() {
        ...
        return
            apply_filters('comment_text', get_comment_text(), $comment);
    }
    ...
    replyLink($linkText) {
        ...
        return
            get_comment_reply_link($a);
    }

    function
    url() {
        return
            get_comment_link($this->id);
    }

    function
    time($d = '', $gmt = FALSE) {
        return
            get_comment_time($d, $gmt);
    }
...
WpLatteCommentAuthorEntity
    ...
    __toString() {
        return
            apply_filters('comment_author', get_comment_author());
    }
    ...
    avatar($size = '96') {
        return
            get_avatar($this->email(), $size);
    }

    function
    email() {
        return
            apply_filters('author_email', get_comment_author_email());
    }

    function
    url() {
        return
            apply_filters('comment_url', get_comment_author_url());
    }
}
...
WpLattePostEntity
    ...
    function
    content($moreLinkText = NULL, $stripteaser = FALSE) {
    ...
    guid() {
        return
            get_the_guid($this->postId);
    }

    function
    excerpt($wordsCount = NULL) {
        if ($wordsCount) {
            if ($this->hasPassword()) {
                return
                    __('There is no excerpt because this is a protected post.', 'wplatte');
            }
            ...
            $excerptLength = apply_filters('excerpt_length', $wordsCount);
    ...
    permalink() {
        return
            apply_filters('the_permalink', get_permalink($this->postId));
    }

    function
    shortUrl() {
        return
            wp_get_shortlink($this->postId);
    }
    ...
    function
    categoryList($separator = '', $parents = '', $taxonomy = '') {
        if (!$this->isCpt) {
            return
                get_the_category_list($separator, $parents, $this->postId);
        }
        else {
            $objectTaxonomies = get_object_taxonomies($this->type);
            $publicObjectTaxonomies = array();
            ...
            if (count($publicObjectTaxonomies) == 1
                or $taxonomy
            ) {
                ...
                return
                    get_the_term_list($this->postId, $tax, '', $separator);
            }
            else {
                $list = array();
                foreach ($publicObjectTaxonomies
                         as $tax) {
    ...
    categories($taxonomy = 'category') {
        ...
        $registeredTaxonomies = array_diff($registeredTaxonomies, array(
            'language',
            'post_translations'
        ));
        if ($this->isCpt
            and $taxonomy == 'category' and
            count($registeredTaxonomies) == 1
        ) {
    ...
    function
    recursiveCategoriesSlugs($id, $type, $taxonomy, $separator, $prefix = "", $suffix = "") {
        $result = "";
        if ($type == "post") {
    ...
    function
    catSlugs($taxonomy, $separator, $prefix = "", $suffix = "") {
        $result = "";
    ...
    function
    categoriesSlugs($prefix = '', $suffix = '', $asString = TRUE, $taxonomy = 'category') {
        $funcParams = func_get_args();
    ...
    hasCategory($category = '') {
        return
            has_term($category, 'category', $this->postId);
    }

    function
    hasTaxonomy($taxonomy) {
        return
            in_array(WpLatteUtils::addPrefix('taxonomy', $taxonomy), get_object_taxonomies($this->type));
    }

    function
    tagList($separator = ', ') {
        return
            get_the_tag_list('', $separator, '', $this->postId);
    }

    function
    hasTag($tag = '') {
        return
            has_term($tag, 'post_tag', $this->postId);
    }

    function
    status() {
        return
            get_post_status($this->postId);
    }
    ...
    formatTitle($format = '') {
        if ($format) {
            ...
            return
                get_post_format_string($this->formatName());
        }
    }
    ...
    formatName() {
        return
            get_post_format($this->postId);
    }

    function
    gallery() {
        return
            get_post_gallery($this->postId, TRUE);
    }
    ...
    function
    image($size = 'post-thumbnail') {
        return
            get_the_post_thumbnail($this->postId == 0 ? NULL : $this->postId, $size);
    }

    function
    date($format = '', $translate = FALSE) {
    ...
    time($d = '') {
        return
            apply_filters('the_time', get_the_time($d, $this->postId), $d);
    }

    function
    dayArchiveUrl() {
        return
            get_day_link(get_the_time('Y', $this->postId), get_the_time('m', $this->postId), get_the_time('d', $this->postId));
    }

    function
    monthArchiveUrl() {
        return
            get_month_link(get_the_time('Y', $this->postId), get_the_time('m', $this->postId));
    }

    function
    yearArchiveUrl() {
        return
            get_year_link(get_the_time('Y', $this->postId));
    }
    ...
    hasPassword() {
        return
            post_password_required($this->postId);
    }

    function
    isSticky() {
        return
            is_sticky($this->postId);
    }

    function
    linkPages($args = array()) {
        if (is_array($args)) {
            $args['echo'] = FALSE;
    ...
    function
    willCommentsPaginate() {
        return (get_comment_pages_count() > 1
            and
    ...
    hasCommentsOpen() {
        return
            comments_open($this->postId);
    }

    function
    hasCommentsClosed() {
        return (!comments_open($this->postId) and
            get_comments_number($this->postId) != '0' and
            post_type_supports(get_post_type($this->postId), 'comments'));
    }
    ...
    mimeType() {
        return
            get_post_mime_type($this->postId);
    }

    function
    meta($metaboxId, $key = NULL) {
        $metaboxKey = "_{$this->type}_{$metaboxId}";
        if ($return = WpLatteObjectCache::load("metabox-{$metaboxKey}-{$key}-{$this->postId}")) {
            return $return;
        }
        $postmeta = get_post_meta($this->postId, $metaboxKey, TRUE);
        if ($postmeta === "") {
            $postmeta = get_post_meta($this->postId, $metaboxId, TRUE);
        }
...
WpLattePostAuthorEntity
    ...
    postsUrl() {
        return
            get_author_posts_url($this->id);
    }

    function
    bio() {
        return
            get_the_author_meta('description', $this->id);
    }
    ...
    meta($metaboxId, $key = NULL) {
        ...
        $usermeta = get_the_author_meta($metaboxKey, $this->id);
        if ($usermeta === "") {
            $usermeta = get_post_meta($metaboxId, $this->id);
        }
    ...
    __toString() {
        return
            apply_filters('the_author', $this->displayName);
    }
}
...
WpLatteTaxonomyTermEntity
    ...
    function
    __construct($term, $taxonomy = 'category') {
        if (is_numeric($term)) {
    ...
    title() {
        return
            apply_filters('single_term_title', $this->rawTitle);
    }
...
WpLatteWpEntity
    ...
    isBlog($page = '') {
        global $wp_query;
        return
            is_home() and $wp_query->is_posts_page;
    }
    ...
    function
    isSingular($postTypes = '') {
        return
            is_singular(WpLatteUtils::addPrefix('post', $postTypes));
    }

    function
    isPostTypeArchive($postTypes = '') {
        return
            is_post_type_archive(WpLatteUtils::addPrefix('post', $postTypes));
    }
    ...
    isTax($taxonomy = '', $term = '') {
        return
            is_tax(WpLatteUtils::addPrefix('taxonomy', $taxonomy), $term);
    }
    ...
    hasPreviousPost() {
        ...
        return (bool) ($post
            and
            is_attachment()) ? get_post($post->post_parent) : get_adjacent_post(FALSE, '', TRUE);
    }
    ...
    function
    isWidgetAreaActive($areaId) {
        return
            is_active_sidebar($areaId);
    }
}
...
WpLatteBreadcrumbs {
    static
    function
    breadcrumbs($args = array()) {
        $breadcrumb = '';
        $defaults = array(
            'container' => 'nav',
            'separator' => '&raquo;',
            'before' => __('You are here:', 'wplatte'),
            'after' => FALSE,
            'frontpage' => TRUE,
            'showHome' => __('Home', 'wplatte'),
            'network' => FALSE
        );
        if (is_singular()) {
            $post = get_queried_object();
            $defaults["singular_{$post->post_type}_taxonomy"] = FALSE;
            $defaults["singular_{$post->post_type}_taxonomy"] = apply_filters('breadcrumbs_singular_taxonomy', FALSE, $post);
        }
        $args = apply_filters('WpLatteBreadcrumbsArgs', $args);
        $args = wp_parse_args($args, $defaults);
        if (function_exists('is_woocommerce') and
            is_woocommerce()
        ) {
            return
                self::woocommerceBreadcrumbs($args);
        }
        $trail = self::getItems($args);
        if (!empty($trail) && is_array($trail)) {
            $breadcrumb = '<' . tag_escape($args['container']) . ' class="breadcrumbs">';
            $breadcrumb .= (!empty($args['before']) ? '<span class="breadcrumbs-before">' . $args['before'] . '</span> ' : '');
            if (1 < count($trail)) {
                array_unshift($trail, '<span class="breadcrumb-begin">' . array_shift($trail) . '</span>');
            }
            array_push($trail, '<span class="breadcrumb-end">' . array_pop($trail) . '</span>');
            $separator = (!empty($args['separator']) ? '<span class="breadcrumb-sep">' . $args['separator'] . '</span>' : '<span class="sep">/</span>');
            $breadcrumb .= join(" {$separator} ", $trail);
            $breadcrumb .= (!empty($args['after']) ? ' <span class="breadcrumbs-after">' . $args['after'] . '</span>' : '');
            $breadcrumb .= '</' . tag_escape($args['container']) . '>';
        }
        $breadcrumb = apply_filters('WpLatteBreadcrumbs', $breadcrumb, $args);
        return $breadcrumb;
    }
    ...
    getItems($args = array()) {
        ...
        $trail = array();
        $path = '';
        if (!is_front_page() && $args['showHome']) {
            if (is_multisite() && $args['network']) {
                $trail[] = '<a href="' . network_home_url() . '">' . $args['showHome'] . '</a>';
                $trail[] = '<a href="' . home_url() . '" title="' . esc_attr(get_bloginfo('name')) . '" rel="home" class="breadcrumb-begin">' . get_bloginfo('name') . '</a>';
            }
            else {
                $trail[] = '<a href="' . home_url() . '" title="' . esc_attr(get_bloginfo('name')) . '" rel="home" class="breadcrumb-begin">' . $args['showHome'] . '</a>';
            }
        }
        if (function_exists('is_bbpress') && is_bbpress()) {
            $trail = array_merge($trail, self::getBBpressItems());
        }
        elseif (is_front_page()) {
            if (!is_paged() && $args['showHome'] && $args['frontpage']) {
                if (is_multisite() && $args['network']) {
                    $trail[] = '<a href="' . network_home_url() . '">' . $args['showHome'] . '</a>';
            ...
            elseif (is_paged() && $args['showHome'] && $args['frontpage']) {
                if (is_multisite() && $args['network']) {
                    $trail[] = '<a href="' . network_home_url() . '">' . $args['showHome'] . '</a>';
                    $trail[] = '<a href="' . home_url() . '" title="' . esc_attr(get_bloginfo('name')) . '" rel="home" class="breadcrumb-begin">' . get_bloginfo('name') . '</a>';
                }
        ...
        elseif (is_home()) {
            ...
            $trail = array_merge($trail, self::getParents($home_page->post_parent, ''));
            if (is_paged()) {
        ...
        elseif (is_singular()) {
            ...
            if ('post' == $post_type) {
                ...
                if (!empty($path)) {
                    $trail = array_merge($trail, self::getParents('', $path));
                }
                $trail = array_merge($trail, self::mapRewriteTags($post_id, get_option('permalink_structure'), $args));
            }
            elseif ('attachment' == $post_type) {
                ...
                if (0 < $parent_id) {
                    ...
                    if ('post' == $parent_post_type) {
                        ...
                        $trail = array_merge($trail, self::mapRewriteTags($post->post_parent, get_option('permalink_structure'), $args));
                    }
            ...
            elseif ('page' !== $post_type) {
                ...
                if (!empty($post_type_object->has_archive) && apply_filters('wplatte-breadcrumbs-enable-cpt-archive', TRUE, $post_type)) {
                    $label = !empty($post_type_object->labels->archive_title) ? $post_type_object->labels->archive_title : $post_type_object->labels->name;
                    //$trail[] = '<a href="' . get_post_type_archive_link($post_type) . '" title="' . esc_attr($label) . '">' . $label . '</a>';
                }
            }
            if ((empty($path) && 0 !== $parent) || ('attachment' == $post_type)) {
                $trail = array_merge($trail, self::getParents($parent, ''));
            }
            ...
            if (!empty($args["singular_{$post_type}_taxonomy"]) && $terms = get_the_term_list($post_id, $args["singular_{$post_type}_taxonomy"], '', ', ', '')) {
                $trail[] = apply_filters('wplatte-breadcrumbs-terms-list', $terms, $post_type, $post_id, $args);
            }
        ...
        elseif (is_archive()) {
            if (is_tax() || is_category() || is_tag()) {
                ...
                if (is_taxonomy_hierarchical($term->taxonomy) && $term->parent) {
                    $trail = array_merge($trail, self::getTermParents($term->parent, $term->taxonomy));
                }
                if (is_paged()) {
            ...
            elseif (is_author()) {
                ...
                if (is_paged()) {
                    $trail[] = '<a href="' . esc_url(get_author_posts_url($user_id)) . '" title="' . esc_attr(get_the_author_meta('display_name', $user_id)) . '">' . __('Author:', 'wplatte') . ' ' . get_the_author_meta('display_name', $user_id) . '</a>';
                }
                else {
                    $trail[] = __('Author:', 'wplatte') . ' ' . get_the_author_meta('display_name', $user_id);
                }
            }
            elseif (is_time()) {
                if (get_query_var('minute') && get_query_var('hour')) {
                    $trail[] = get_the_time(__('g:i a', 'wplatte'));
                }
                elseif (get_query_var('minute')) {
                    $trail[] = sprintf(__('Minute %1$s', 'wplatte'), get_the_time(__('i', 'wplatte')));
                }
                elseif (get_query_var('hour')) {
                    $trail[] = get_the_time(esc_html_x('g a', 'hour archives time format', 'wplatte'));
                }
            }
            elseif (is_date()) {
                if ($wp_rewrite->front) {
                    $trail = array_merge($trail, self::getParents('', $wp_rewrite->front));
                }
                if (is_day()) {
                    $trail[] = '<a href="' . get_year_link(get_the_time('Y')) . '" title="' . get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte')) . '">' . get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte')) . '</a>';
                    $trail[] = '<a href="' . get_month_link(get_the_time('Y'), get_the_time('m')) . '" title="' . get_the_time(esc_html_x('F', 'monthly archives date format', 'wplatte')) . '">' . get_the_time(__('F', 'wplatte')) . '</a>';
                    if (is_paged()) {
                        $trail[] = '<a href="' . get_day_link(get_the_time('Y'), get_the_time('m'), get_the_time('d')) . '" title="' . get_the_time('d') . '">' . get_the_time('d') . '</a>';
                    }
                    else {
                        $trail[] = get_the_time('d');
                    }
                }
                elseif (get_query_var('w')) {
                    ...
                    if (is_paged()) {
                        $trail[] = get_archives_link(add_query_arg(array(
                            'm' => get_the_time('Y'),
                            'w' => get_the_time('W')
                        ), home_url()), sprintf(__('Week %1$s', 'wplatte'), get_the_time(esc_html_x('W', 'weekly archives date format', 'wplatte'))), FALSE);
                    }
                    else {
                        $trail[] = sprintf(__('Week %1$s', 'wplatte'), get_the_time(esc_html_x('W', 'weekly archives date format', 'wplatte')));
                    }
                }
                elseif (is_month()) {
                    $trail[] = '<a href="' . get_year_link(get_the_time('Y')) . '" title="' . get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte')) . '">' . get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte')) . '</a>';
                    if (is_paged()) {
                        ...
                        $trail[] = get_the_time(__('F', 'wplatte'));
                    }
                }
                elseif (is_year()) {
                    if (is_paged()) {
                        $trail[] = '<a href="' . get_year_link(get_the_time('Y')) . '" title="' . esc_attr(get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte'))) . '">' . get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte')) . '</a>';
                    }
                    else {
                        $trail[] = get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte'));
                    }
                }
            }
        }
    ...
    getBBpressItems($args = array()) {
        ...
        elseif (bbp_is_single_topic()) {
            ...
            $trail = array_merge($trail, self::getParents(bbp_get_topic_forum_id($topic_id)));
            ...
            if (bbp_is_topic_split()) {
                $trail[] = __('Split', 'wplatte');
            }
            elseif (bbp_is_topic_merge()) {
                $trail[] = __('Merge', 'wplatte');
            }
        ...
        elseif (bbp_is_single_reply()) {
            ...
            $trail = array_merge($trail, self::getParents(bbp_get_reply_topic_id($reply_id)));
        ...
        elseif (bbp_is_single_forum()) {
            ...
            if (0 !== $forum_parent_id) {
                $trail = array_merge($trail, self::getParents($forum_parent_id));
            }
        ...
        return
            apply_filters('WpLatteBreadcrumbsBBpressItems', $trail, $args);
    }

    static
    function
    mapRewriteTags($post_id = '', $path = '', $args = array()) {
        $trail = array();
        ...
        if (is_array($matches)) {
            foreach ($matches
                ...
                $tag = trim($match, '/');
                if ('%year%' == $tag) {
                    $trail[] = '<a href="' . get_year_link(get_the_time('Y', $post_id)) . '" title="' . get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte'), $post_id) . '">' . get_the_time(esc_html_x('Y', 'yearly archives date format', 'wplatte'), $post_id) . '</a>';
                }
                elseif ('%monthnum%' == $tag) {
                    $trail[] = '<a href="' . get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id)) . '" title="' . get_the_time(esc_attr__('F Y', 'wplatte'), $post_id) . '">' . get_the_time(__('F', 'wplatte'), $post_id) . '</a>';
                }
                elseif ('%day%' == $tag) {
                    $trail[] = '<a href="' . get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id)) . '" title="' . get_the_time(esc_attr__('F j, Y', 'wplatte'), $post_id) . '">' . get_the_time('d', $post_id) . '</a>';
                }
                elseif ('%author%' == $tag) {
                ...
                elseif ('%category%' == $tag && 'category' !== $args["singular_{$post->post_type}_taxonomy"]) {
                    ...
                    if ($terms) {
                        ...
                        if (0 !== $term->parent) {
                            $trail = array_merge($trail, self::getTermParents($term->parent, 'category'));
                        }
    ...
    static
    function
    getTermParents($parent_id = '', $taxonomy = '') {
        $trail = array();
        $parents = array();
    ...
    static
    function
    woocommerceBreadcrumbs($args) {
        $breadcrumbs = new
        WC_Breadcrumb();
        if ($args['showHome']) {
            $breadcrumbs->add_crumb($args['showHome'], apply_filters('woocommerce_breadcrumb_home_url', home_url()));
            if (!is_shop()) {
                $shopPage = get_page(woocommerce_get_page_id('shop'));
                $breadcrumbs->add_crumb(get_the_title($shopPage), get_permalink($shopPage));
            }
        }
        $trails = $breadcrumbs->generate();
        $output = '';
        $output .= '<' . tag_escape($args['container']) . ' class="breadcrumbs">';
        $output .= (!empty($args['before']) ? '<span class="breadcrumbs-before">' . $args['before'] . '</span> ' : '');
        foreach ($trails
                 as $i => $trail) {
            if ($i === 0) {
                $output .= '<span class="breadcrumb-begin">';
            }
            elseif (empty($trail[1]) and
                count($trails) === ($i + 1)
            ) {
                $output .= '<span class="breadcrumb-end">';
            }
            if (!empty($trail[1]) and
                count($trails) !== ($i + 1)
            ) {
                $output .= '<a href="' . esc_url($trail[1]) . '">' . esc_html($trail[0]) . '</a>';
            }
            else {
                $output .= esc_html($trail[0]);
            }
            if ($i === 0) {
                $output .= '</span>';
            }
            ...
            if (count($trails) !== ($i + 1)) {
                $output .= (!empty($args['separator']) ? ' <span class="breadcrumb-sep">' . $args['separator'] . '</span> ' : ' <span class="sep">/</span> ');
            }
        }
        $output .= '</' . tag_escape($args['container']) . '>';
        return $output;
    }
}

if (!class_exists('NPresenter')) {
    class
    NPresenter {
    }
}
...
WpLatteFileTemplate
    ...
    render() {
        ...
        $cache = new
        NCache($storage = $this->getCacheStorage(), 'wplatte');
        if ($storage
            instanceof
            NPhpFileStorage
        ) {
        ...
        if ($compiled === NULL) {
            ...
            $cache->save($this->getFile(), $compiled, array(
                ...
                NCache::CONSTS => apply_filters('wplatte-cache-constants', array(
                    'NFramework::REVISION',
                    'WPLATTE_VERSION'
                ))
            ));
...
WpLatteLoopIterator
    ...
    current() {
        ...
        global $post;
        return
            WpLatte::createEntity('Post', $post);
    }
    ...
    isLast($width = NULL) {
        ...
        return !$hasNext || ($width && ($this->counter
                % $width) === 0);
    }
    ...
    __call($name, $args) {
        return
            NObjectMixin::call($this, $name, $args);
    }

    function&__get($name) {
        return
            NObjectMixin::get($this, $name);
    }
...
WpLatteMacros
    ...
    static
    function
    install(NLatteCompiler $compiler, $config = NULL) {
        $me = new
        self($compiler);
        $me->config = $config;
        $me->addMacro('_', array($me, 'macroTranslate'), array(
            $me,
            'macroTranslate'
        ));
        ...
        $me->addMacro('loop', array($me, 'macroLoop'), array(
            $me,
            'macroLoopEnd'
        ));
        $me->addMacro('customQuery', array($me, 'macroCustomQuery'));
        $me->addMacro('customLoop', array($me, 'macroCustomLoop'), array(
            $me,
            'macroLoopEnd'
        ));
    ...
    function
    macroIncludePart(NMacroNode $node, NPhpWriter $writer) {
        ...
        return $writer->write('NCoreMacros::includeTemplate(' . __CLASS__ . '::getTemplatePart(' . $slug . ', ' . $name . '), ' . $params['params'] . ' + get_defined_vars(), $_l->templates[%var])->render()', $this->getCompiler()
            ->getTemplateId());
    }

    function
    macroTranslate(NMacroNode $node, $writer) {
        $fn = '__  (';
        if ($node->closing) {
    ...
    function
    macroLoopEnd(NMacroNode $node, NPhpWriter $writer) {
        $code = 'endforeach;';
    ...
    function
    macroLoopComments(NMacroNode $node, NPhpWriter $writer) {
        $node->tokenizer->fetchWord();
    ...
    function
    menu($location, $args) {
        $defaults = array('theme_location' => $location);
    ...
    breadcrumbs($args) {
        ...
        return
            WpLatteBreadcrumbs::breadcrumbs($args);
    }

    static
    function
    pagination($args = array()) {
    ...
    prepareCustomWpQuery($args) {
        ...
        return
            new
            WpLatteWpQuery($query);
    }

    static
    function
    getTemplatePart($slug, $name = NULL) {
...
WpLatteObjectCache {
    ...
    static
    function
    save($key, $data, $args = array()) {
        if ($args) {
            $args[] = $key;
            $key = self::cacheKey($args);
        }
        return
            wp_cache_add($key, $data, self::$group);
    }
...
WpLatteTemplateHelpers {
    ...
    static
    function
    stripTagsExcept($string, $allowedTags = '') {
        $string = preg_replace('@<(script|style)[^>]*?>.*?</\\1>@si', '', $string);
    ...
    static
    function
    trimWords($string, $numberOfWords = 55, $more = NULL) {
        if ($more === NULL) {
            $more = __('&hellip;', 'wplatte');
        }
        $originalString = $string;
        if (_x('words', 'word count: "words" or "characters"?', 'wplatte') == 'characters' and
        ...
        return
            apply_filters('wp_trim_words', $string, $numberOfWords, $more, $originalString);
    }

    static
    function
    date($rawMySqlDate, $format = '') {
        ...
        return
            mysql2date($format, $rawMySqlDate, FALSE);
    }

    static
    function
    dateI18n($rawMySqlDate, $format = '') {
        if (empty($rawMySqlDate)) {
            return '';
        }
        if (!$format) {
...
WpLatteTemplateHierarchy {
    ...
    register() {
        ...
        add_filter('attachment_template', array(
            __CLASS__,
            'singularTemplate'
        ), 11);
    }
    ...
    userTemplate($template) {
        ...
        $user = new
        WP_User(absint(get_query_var('author')));
    ...
    taxonomyTemplate($template) {
        ...
        return
            locate_template(array(
                "taxonomy-{$originalTax}-{$slug}.php",
                "taxonomy-{$originalTax}.php",
                "taxonomy-{$unprefixedTax}-{$slug}.php",
    ...
    static
    function
    singularTemplate($template) {
        $templates = array();
        $post = get_queried_object();
        $originalType = $post->post_type;
        $unprefixedType = WpLatteUtils::stripPrefix('post', $post->post_type);
        $custom = get_post_meta(get_queried_object_id(), "_wp_{$unprefixedType}_template", TRUE);
        if (!$custom) {
...
WpLatteUtils {
    protected
    static $cptPrefixes = array('post' => '', 'taxonomy' => '');
    protected
    ...
    static
    function
    needsPrefix($item, $type) {
        if ($type == 'taxonomy') {
            return
                self::isSpecificCustomTax($item);
        }
        return
            self::isSpecificCpt($item);
    }
    ...
    isSpecificCpt($type) {
        ...
        return
            isset($cpts["{$prefix}{$type}"]);
    }

    static
    function
    isSpecificCustomTax($tax) {
...
WpLatteWpQuery
    ...
    __get($name) {
        ...
        elseif (method_exists($this, $fn) and
